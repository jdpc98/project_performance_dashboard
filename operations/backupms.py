    ...
    dcc.Tab(label='Reports', value='tab-reports', children=[
        html.Div([
            ...
            html.Div([
                ...
                # Second table: Projects with ER values only
                html.H3("Projects with ER Invoiced Values Only", style={'textAlign': 'center', 'marginTop': '30px'}),
                dash_table.DataTable(
                    id='weekly-report-table-er',
                    columns=[],
                    data=[],
                    style_table={'width': '95%', 'margin': 'auto', 'overflowX': 'auto'},
                    style_cell={'textAlign': 'center', 'fontFamily': 'Calibri, sans-serif'},
...
@app.callback(
    [Output('weekly-report-table', 'data'),
     ...
     Output('weekly-report-table-er', 'data'),
     Output('weekly-report-table-er', 'columns')],
    Input('report-week-picker', 'date')
)
def generate_monthly_report(selected_date):
    ...
    try:
        ...
        df_all = pd.DataFrame(active_project_details)

        # For the second table: Filter to show only projects with valid ER values
        df_with_er = df_all.copy()

        # Filter to keep only rows with numeric ER Invoiced values
        def is_valid_er(val):
            try:
        ...
        # Apply the filter for ER table
        if 'ER Invoiced' in df_with_er.columns:
            df_with_er = df_with_er[df_with_er['ER Invoiced'].apply(is_valid_er)]

        # If no projects with ER, return empty data for second table
        er_data = df_with_er.to_dict('records') if not df_with_er.empty else []
        ...
        for record in er_data:
            if 'Original_Order' in record:
                del record['Original_Order']

        print_green(f"Final report contains {len(active_project_details)} projects, with {len(er_data)} having valid ER values")
        print_green(f"==================== END OF REPORT GENERATION ====================")

        return active_project_details, columns, er_data, columns
...
def main():
    ...
    for year in years:
        ...
        df_projects = get_project_log_data(year=year)
