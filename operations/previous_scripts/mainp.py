...
# Shut up all warnings (adjust if you need specific ones later)
warnings.simplefilter("ignore")

# Global variables that our callbacks will use (like a shared notebook)
global_merged_df = None
global_projects_df = None
global_invoices = None  # For our new invoices data

# ----- Style settings for our Dash tables -----
TABLE_STYLE = {
    'overflowX': 'auto',
    'width': '100%',
...
# Function to print in green (for fun and debugging clarity)
def print_green(message):
    print("\033[92m" + message + "\033[0m")

# Load and encode the logo image – updated path here!
...
def trm_ingestion(rates_df):
    ...
    # Grab TRM, Bogota, and Houston values from columns 4 to 29 (0-indexed)
    trm_values = df_rates.iloc[0, 4:29].values
    bogota_vals = df_rates.iloc[1, 4:29].values
    houston_vals = df_rates.iloc[2, 4:29].values

    # Rows 4 and 5 have date parts; combine them into one string per column.
...
def rates_ingestion(rates_df):
    ...
    # Prepend our constant columns to the date columns (25 of them) to get 29 columns total.
...
def calculate_day_cost(merged_df):
    # Convert 'local_date' into a datetime object.
    ...
    def row_day_cost(row):
        ...
        if pd.isnull(dt):
            return 0
        year = dt.year
        # For July, we pick a different column name based on the day.
        if dt.month == 7:
...
def assign_total_hours(merged_df):
    # Create separate columns for 2024 and 2025 hours.
...
def load_third_file_dynamic(third_file):
    ...
    nrows = len(df_raw)
    end_row = None
    # Look for two consecutive empty rows in column B after row 630.
...
def main():
    ...
    # 1) Load the RATES data from our single "Rates" sheet.
    ...
    ############
    # Force the 'TotalInvoice' column to be numeric, just in case it was read as string
    ...
    # 8) Instead of multiple files, we export one Excel file with multiple sheets (one for each jobcode).
...
app.layout = html.Div("Project Dashboard"[
    # Logo at the top.
    html.Div([
...
def update_project_tables(selected_jobcode):
    ...
    if not filtered.empty:
        ...
        # Look up real invoice from global_invoices for this project.
        ...
        for key, value in right_data.items():
            if value is not None and isinstance(value, (int, float)):
                ...
                elif 'ER Contract' in key or 'ER Invoiced' in key:
                    formatted_right_data[key] = f"{value:.2f}"
                else:
                    formatted_right_data[key] = f"${value:,.2f}"
            else:
