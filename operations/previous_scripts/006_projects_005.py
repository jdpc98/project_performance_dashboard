...
# ===== NEW CALLBACK: SERVICE ITEM PIE CHARTS =====
@app.callback(
...
def update_service_item_pie_charts(selected_jobcode, selected_years):
    ...
    df_filtered = global_merged_df[global_merged_df['jobcode_2'] == selected_jobcode].copy()
    print_green(f"Filtered DF shape after jobcode filter: {df_filtered.shape}")

    if selected_years:
        try:
            selected_years_int = [int(y) for y in selected_years]
            df_filtered = df_filtered[df_filtered['local_date'].dt.year.isin(selected_years_int)]
            print_green(f"Filtered DF shape after year filter: {df_filtered.shape}")
        except Exception as e:
            print_green("Error filtering by year: " + str(e))

    if df_filtered.empty:
        default_fig = px.pie(title="No data available after filtering")
        return default_fig, default_fig

    # Attempt to find the "service item" column.
    service_item_col = None
    for col in df_filtered.columns:
        if col.lower().replace("_", " ").strip() == "service item":
            service_item_col = col
            break
    # Fallback: try any column with 'service' in its name.
    if service_item_col is None:
        for col in df_filtered.columns:
            if "service" in col.lower():
                service_item_col = col
                print_green(f"Falling back to column: {service_item_col}")
                break
    ...
    grouped = df_filtered.groupby(service_item_col, as_index=False).agg({'hours': 'sum', 'day_cost': 'sum'})
    print_green("Grouped data:")
    print_green(str(grouped.head()))

    if grouped.empty:
        default_fig = px.pie(title="No data after grouping")
        return default_fig, default_fig

    # Pie chart for total hours per service item.
    fig_hours = px.pie(
        grouped,
    ...
    # Pie chart for total cost per service item.
    fig_cost = px.pie(
        grouped,
        names=service_item_col,
        values='day_cost',
    ...
    fig_hours.update_layout(title_font=dict(family="Calibri, sans-serif"))
    fig_cost.update_layout(title_font=dict(family="Calibri, sans-serif"))
...
# ===== CALLBACKS FOR "OTHER" DROPDOWN TOGGLE (for Add New Project tab) =====
