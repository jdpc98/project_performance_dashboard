...
# Suppress known openpyxl warnings.
...
RIGHT_TABLE_RED_STYLE = [
    {
        'if': {
            'filter_query': '{Field} = "Total Invoice (Temporarily Unavailable)"',
            'column_id': 'Value'
        },
        'color': 'red'
    },
    {
        'if': {
            'filter_query': '{Field} = "Remaining to be invoiced (Temporarily Unavailable)"',
            'column_id': 'Value'
        },
        'color': 'red'
    },
...
def load_third_file_dynamic(third_file):
    """
    Reads the entire '4_Contracted Projects' sheet without a header.
    Starting at row 630, looks for two consecutive empty rows in column B.
    Truncates the DataFrame there. Then it uses the first row as the header,
    drops any duplicate header rows in the data, and cleans the column names.
    """
    df_raw = pd.read_excel(third_file, sheet_name='4_Contracted Projects', header=None, engine='openpyxl')
    nrows = len(df_raw)
    end_row = None
    # Look for two consecutive empty rows in column B (index 1) starting from row 630.
    ...
    # Use the first row as header and clean it.
    header = [str(col).strip() for col in df_trunc.iloc[0].tolist()]
    df_data = df_trunc.iloc[1:].copy()
    # Drop any row that exactly matches the header (duplicate header rows)
...
app.layout = html.Div([
    ...
    # Dropdown for selecting Jobcode.
    html.Div([
...
# Callback to update jobcode-dropdown options based on filters.
@app.callback(
...
# Callback for the first (Time Distribution) pie chart.
@app.callback(
...
def update_pie_chart(selected_jobcode, selected_years):
    ...
    fig.update_layout(
        title={'x': 0.5},
        legend=dict(orientation="v", x=1.1, y=0.5, xanchor="left", yanchor="middle")
    )
    return fig

# Callback for the second (Cost Distribution) pie chart.
@app.callback(
...
# Callback for updating project tables.
@app.callback(
