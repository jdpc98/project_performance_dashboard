...
def main():
    ...
    ###explore results with both separate and merged, only separate used until now. 
...
app.layout = html.Div([
    html.H1("DECON Project Performance", style={'textAlign': 'center'}),
    html.Div([
        html.Div([
    ...
    html.Div([
        html.Div([
            ...
            # Left table: transposed descriptive fields.
...
def update_pie_chart(selected_jobcode, selected_years):
    ...
    fig = px.pie(
        ...
        names='Personel',
        values=value_col,
        title=f"Percentage of Hours by Personel: {selected_jobcode}"
    )
...
def update_cost_pie_chart(selected_jobcode, selected_years):
    ...
    fig = px.pie(
        ...
        names='Personel',
        values='day_cost',
        title=f"Cost Distribution by Personel: {selected_jobcode}<br>Total Cost: ${total_cost:,.2f}"
    )
    fig.update_layout(
        title={'x': 0.5},
        legend=dict(
    ...
    fig.update_traces(hovertemplate='<b>%{label}</b><br>Total Employee Cost: %{value:$,.2f}<extra></extra>')
    return [fig]
...
def update_project_tables(selected_jobcode):
    ...
    if not filtered.empty:
        ...
        # Right table: cost and contract fields.
        ...
        for key, value in right_data.items():
            if value is not None and isinstance(value, (int, float)):
                if key in ['ER Contract', 'ER Invoiced']:
