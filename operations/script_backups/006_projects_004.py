...
app.layout = dcc.Tabs(id='tabs-example', value='tab-dashboard', children=[
    dcc.Tab(label='Dashboard', value='tab-dashboard', children=[
        html.Div([
            ...
            html.H2("Service Item Distribution", style={'textAlign': 'center', 'paddingTop': '20px'}),
            html.Div(dcc.Graph(id='service-item-pie-chart'), style={'width': '60%', 'margin': '0 auto'}),
...
##callback for service item pie chart


@app.callback(
    Output('service-item-pie-chart', 'figure'),
    [Input('jobcode-dropdown', 'value'),
     Input('year-dropdown', 'value')]
)
def update_service_item_pie_chart(selected_jobcode, selected_years):
    if selected_jobcode is None:
        return {}
    
    # Filter the global dataframe based on selected jobcode and year(s)
    df_filtered = global_merged_df[global_merged_df['jobcode_2'] == selected_jobcode].copy()
    if selected_years:
        selected_years_int = [int(y) for y in selected_years]
        df_filtered = df_filtered[df_filtered['local_date'].dt.year.isin(selected_years_int)]
    
    # Identify the service item column (using the same flexible check as in the table callback)
    service_item_col = None
    for col in df_filtered.columns:
        if col.lower().replace("_", " ").strip() == "service item":
            service_item_col = col
    ...
    # Group by service item and sum the 'hours' (or you can choose 'day_cost' if desired)
    grouped = df_filtered.groupby(service_item_col, as_index=False).agg({'hours': 'sum', 'day_cost': 'sum'})
    ...
    fig = px.pie(
        ...
        values='hours',
        title=f"Service Item Distribution for Jobcode: {selected_jobcode}<br><b>Total Hours:</b> {total_hours:.2f}"
    )
