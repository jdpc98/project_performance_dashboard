...
# Global variables (our shared notebook for callbacks)
global_merged_df = None
global_projects_df = None
global_invoices = None  # Invoices data backup
...
# Helper function for green terminal prints (for debugging fun)
def print_green(message):
    print("\033[92m" + message + "\033[0m")

# Load and encode our logo (updated path)
...
def trm_ingestion(rates_df):
    ...
    # Combine date parts from rows 4 and 5 into one string per column.
...
def calculate_day_cost(merged_df):
    ...
    def row_day_cost(row):
        ...
        if pd.isnull(dt):
            return 0
        year = dt.year
        # For July, choose column based on day.
        if dt.month == 7:
...
def main():
    ...
    # 1) Load Rates data from the "Rates" sheet.
    ...
    # 3) Merge timesheet with rates (mapping Employee to ID#)
    ...
    # 4) Load Projects data dynamically.
    ...
    # 9) Save final merged and projects data globally for Dash callbacks.
...
# ======= NEW CALLBACK FOR SERVICE ITEM DETAILS =======
@app.callback(
...
def update_service_item_table(selected_jobcode, selected_years):
    # If no jobcode selected, return empty table
    if selected_jobcode is None:
        return [], []
    # Filter merged data by selected jobcode
    ...
    # Group by the 'service item' field, summing hours and day_cost.
    # (Make sure the column name exactly matches your data; here we use "service item".)
    if 'service item' in df_filtered.columns:
        grouped = df_filtered.groupby('service item', as_index=False).agg({'hours': 'sum', 'day_cost': 'sum'})
        # Format cost as dollars and hours to two decimals.
        grouped['day_cost'] = grouped['day_cost'].apply(lambda x: f"${x:,.2f}")
        grouped['hours'] = grouped['hours'].apply(lambda x: f"{x:.2f}")
        columns = [
            {'name': 'Service Item', 'id': 'service item'},
    ...
    dcc.Tabs(id='tabs-example', value='tab-dashboard', children=[
        dcc.Tab(label='Dashboard', value='tab-dashboard', children=[
            ...
            html.Div([
                ...
                # New table for service item details.
        ...
        dcc.Tab(label='Add New Project', value='tab-add', children=[
            # ---- Add New Project Form (Vertical Layout) ----
            html.Div([
...
def add_new_project(n_clicks, proj_no, status_dd, status_other, type_dd, type_other,
    ...
    if not n_clicks:
        return ""
    # Use text input if "Other" is selected.
    ...
    # Append the new project using pd.concat (since .append is deprecated)
    ...
    # After adding, update backup files.
    export_data_safety()
...
def update_service_item_table(selected_jobcode, selected_years):
    ...
    if selected_years:
        selected_years_int = [int(y) for y in selected_years]
        df_filtered = df_filtered[df_filtered['local_date'].dt.year.isin(selected_years_int)]
    # Group by "service item" and sum hours and day_cost
    if 'service item' in df_filtered.columns:
        grouped = df_filtered.groupby('service item', as_index=False).agg({'hours': 'sum', 'day_cost': 'sum'})
        # Format day_cost as dollars and hours to two decimals.
        grouped['day_cost'] = grouped['day_cost'].apply(lambda x: f"${x:,.2f}")
        grouped['hours'] = grouped['hours'].apply(lambda x: f"{x:.2f}")
        columns = [
            {'name': 'Service Item', 'id': 'service item'},
...
# ========= EXISTING CALLBACKS FOR DASHBOARD ==========
@app.callback(
