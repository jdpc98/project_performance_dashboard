...
app.layout = html.Div([
    ...
    html.Div([
        ...
        # Personnel dropdown commented out for now.
        # html.Div([
    ...
    html.Div([
        html.Div([
            ...
            dash_table.DataTable(
                ...
                columns=[{'name': 'Status', 'id': 'Status'},
                         ...
                         {'name': 'Open', 'id': 'Open'},
                         {'name': 'Invoice Completion', 'id': 'Invoice Completion'},
                         {'name': 'Remaining to be invoiced', 'id': 'Remaining to be invoiced'},
        ...
        ], style={'width': '60%', 'display': 'inline-block', 'verticalAlign': 'top', 'margin': '0 auto'})
    ], style={'textAlign': 'center'})
])
...
def update_pie_chart(selected_jobcode, selected_years):
    ...
    fig.update_layout(title={'text': "% of H. by Employee", 'x': 0.5, 'xanchor': 'center'})
...
def update_project_table(selected_jobcode):
    ...
    if not filtered.empty:
        # Define desired columns, now including "ER"
        desired_columns = ['Status', 'Type', 'Service Line', 'Market Segment', 
                           'Project Manager', 'Contracted Amount', 'Project Description', 
                           'Open', 'Invoice Completion', 'Remaining to be invoiced', 'Clients', 'ER']
        available_columns = [col for col in desired_columns if col in filtered.columns]
        if available_columns:
            # If there's only one record, optionally transpose to display vertically.
            if len(filtered) == 1:
                transposed = filtered[available_columns].T.reset_index()
