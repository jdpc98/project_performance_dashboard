...
global_employee_cost_df = None  # for storing aggregated cost data if needed
...
def main():
    ...
    # Create mapping from df_rates_24_25: Personel -> ID#
    ...
    # Export merged DataFrames for verification.
...
app.layout = html.Div([
    ...
    html.Div([
        html.Div([
            ...
            # Left Table: static fields
            dash_table.DataTable(
                id='project-table-left',
                columns=[{'name': 'Clients', 'id': 'Clients'},
                         {'name': 'Type', 'id': 'Type'},
        ...
        html.Div([
            ...
            dash_table.DataTable(
                ...
                columns=[{'name': 'Contracted Amount', 'id': 'Contracted Amount'},
...
def update_project_tables(selected_jobcode):
    ...
    if not filtered.empty:
        # Left table columns.
        left_cols = ['Clients', 'Type', 'Status', 'Service Line', 'Market Segment', 'Project Manager']
        left_available = [col for col in left_cols if col in filtered.columns]
        left_table = filtered[left_available] if left_available else pd.DataFrame()
        
        # Right table columns.
        right_cols = ['Contracted Amount']
        # For Total Cost, we compute it from merged data:
        df_cost = global_merged_df[global_merged_df['jobcode_2'] == selected_jobcode]
        total_cost = df_cost['day_cost'].sum()
        # Create a DataFrame with one row for "Total Cost"
        right_table = pd.DataFrame({
            'Contracted Amount': filtered[right_cols[0]] if right_cols[0] in filtered.columns else [None],
            'Total Cost': [total_cost]
        })
        
        left_columns = [{'name': col, 'id': col} for col in left_available]
        right_columns = [{'name': col, 'id': col} for col in right_table.columns]
        left_data = left_table.to_dict('records')
        right_data = right_table.to_dict('records')
        return left_data, left_columns, right_data, right_columns
