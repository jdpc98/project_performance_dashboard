...
app.layout = html.Div([
    # Logo at the top.
    html.Div([
        html.Img(src='data:image/png;base64,{}'.format(encoded_logo), style={'height': '100px'})
    ...
    # Project Description block with extra margins and bold label.
...
def update_pie_chart(selected_jobcode, selected_years):
    ...
    fig = px.pie(
        ...
        names='Personel',
        values=value_col,
        title=f"Percentage of Hours by Personel for Jobcode: {extract_project_no(selected_jobcode)}<br>Total Hours: {total_hours:.2f}"
    )
...
def update_cost_pie_chart(selected_jobcode, selected_years):
    ...
    fig = px.pie(
        ...
        names='Personel',
        values='day_cost',
        title=f"Cost Distribution by Personel for Jobcode: {extract_project_no(selected_jobcode)}<br>Total Cost: ${total_cost:,.2f}"
    )
    fig.update_layout(
        title={'x': 0.5},
        legend=dict(
